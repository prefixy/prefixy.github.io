...
**This is the story of how Prefixy was built, a highly scalable, query optimized, hosted prefix search service for building autocomplete suggestions.**
...
![terminology](/assets/terms.gif)
**##open####open##** update image **##open####open##**
...
**##open####open##** naive hash image **##open####open##**
...
**##open####open##** searching naive hash img **##open####open##**
...
There’s one last optimization we can make. We can map the trie’s prefixes with the completions stored at each node to a hash in a data structure that has been referred to as a Prefix Hash Tree (our confidence in this approach was bolstered by [this paper](https://people.eecs.berkeley.edu/~sylvia/papers/pht.pdf) which used utilized this to distribute a trie).
...
**##open####open##** code snippet **##open####open##**
...
**##open####open##** **##open####open##**
(authentication middleware code snippet)
...
In MongoDB, we allocate a collection for each tenant ID. Now we can call the `findOneAndUpdate` method on a collection, in order to insert data into a particular collection. This method ensures that all of the app developer’s data is inserted only within the context of her own allocated collection.
...
**##open####open##** **##open####open##**
(MULTI-TENANCY ON THE BACK-END diagram)
...
## References
**##open####open##** **##open####open##**
Antirez Redis Autocomplete article
Prefix Hash Tree paper
